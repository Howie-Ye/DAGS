...
class DepthVideo:
    ...
    def __init__(self, cfg, printer):
        ...
        self.images = torch.zeros(buffer, 3, ht, wd, device=self.device, dtype=torch.float32)
        ...
        self.prior = torch.zeros(buffer, ht, wd, device=self.device, dtype=torch.float).share_memory_()

        # TODO mono depth related items will be removed
    ...
    def get_depth_scale_and_shift(self, index, mono_depth: torch.Tensor, est_depth: torch.Tensor,
    ...
    def get_depth_and_pose(self, index, device):
        with self.get_lock():
            ...
            c2w, w2c = self.get_pose(index, device)
        return est_depth, depth_mask, c2w, w2c
    ...
    def save_video(self, path: str):
        ...
        for i in range(self.counter.value):
            depth, depth_mask, pose, w2c = self.get_depth_and_pose(i, 'cpu')
