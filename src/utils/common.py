...
def select_uv(i, j, n, depth, color, normal, mask, device='cuda:0'):
    """
    ...
    return i, j, depth, color, normal

def get_sample_uv(H0, H1, W0, W1, n, depth, color, normal, mask, device='cuda:0'):
    """
    ...
    i, j = torch.meshgrid(torch.linspace(W0, W1-1, W1-W0).to(device),
                          torch.linspace(H0, H1-1, H1-H0).to(device), indexing='ij')
    i = i.t()
    j = j.t()
    i, j, depth, color, normal = select_uv(i, j, n, depth, color, normal, mask,  device=device)
    return i, j, depth, color, normal
...
def get_samples(H0, H1, W0, W1, n, fx, fy, cx, cy, c2w, depth, color, normal, device,
    ...
    """
    i, j, sample_depth, sample_color, sample_normal = get_sample_uv(
        H0, H1, W0, W1, n, depth, color, normal, mask, device=device)
    ...
    if depth_filter:
        ...
        rays_o, rays_d, sample_depth, sample_color, sample_normal = rays_o[
            mask], rays_d[mask], sample_depth[mask], sample_color[mask], sample_normal[mask]
        i, j = i[mask], j[mask]

    if return_index:
        return rays_o, rays_d, sample_depth, sample_color, sample_normal, i.to(torch.int64), j.to(torch.int64)
    return rays_o, rays_d, sample_depth, sample_color, sample_normal
