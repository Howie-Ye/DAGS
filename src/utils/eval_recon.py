...
from src.utils.cull_mesh import cull_mesh
...
def get_align_transformation(rec_meshfile, gt_meshfile):
    ...
    print('reg_p2p.fitness', reg_p2p.fitness)
    print('reg_p2p.inlier_rmse',reg_p2p.inlier_rmse)
...
def eval_recon_with_cfg(cfg,output,traj_est,eval_3d=True,eval_2d=True, kf_mesh=True, every_mesh=False, printer=None):
    scene_name = cfg["scene"]
    gt_mesh = cfg['meshing']['gt_mesh_path']
    cull_gt_mesh = gt_mesh.replace('.ply', f'_cull.ply')
    if not os.path.exists(cull_gt_mesh):
        cam_info = cfg['cam']
        intrin = (cam_info['H'],cam_info['W'],cam_info['fx'],cam_info['fy'],cam_info['cx'],cam_info['cy'])
        cull_mesh(input_mesh=gt_mesh,traj=traj_est,output_mesh=cull_gt_mesh,intrin=intrin)
    result = {}
    if kf_mesh:
        mesh_name = f'{scene_name}_kf.ply'
        rec_mesh = f'{output}/mesh/{mesh_name}'

        result_kf = eval_recon(rec_mesh,cull_gt_mesh,eval_2d,eval_3d,align=True,printer=printer)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--output", type=str, help="Path to output dir.",
    )
    args = parser.parse_args()
    from src import config
    cfg = config.load_config(args.config, "configs/mono_point_slam.yaml")

    eval_recon_with_cfg(cfg,args.output,printer=TrivialPrinter())
