...
def load_mono_depth(idx,cfg):
    dir_path = f"{cfg['data']['output']}/{cfg['scene']}/mono_priors/depths"
...
class TUM_RGBD(BaseDataset):
    def __init__(self, cfg, device='cuda:0'
        ...
        stride = cfg['stride']
        max_frames = cfg['max_frames']
        # changed [:max_frames][::stride] --> [::stride] if max_frames == -1
        # max_frames=-1 as default
        if max_frames==-1:
            self.color_paths = self.color_paths[::stride]
            self.depth_paths = self.depth_paths[::stride]
            self.poses = self.poses[::stride]
        else:
            self.color_paths = self.color_paths[:max_frames+1][::stride]
            self.depth_paths = self.depth_paths[:max_frames+1][::stride]
            self.poses = self.poses[:max_frames+1][::stride]
        self.n_img = len(self.color_paths)
