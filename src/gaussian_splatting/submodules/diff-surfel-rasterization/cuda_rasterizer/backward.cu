	...
	float* dL_dview)
{
	...
	if (Ts_precomp == nullptr) {
		// Calculate dL_dview from dL_dnormals
		atomicAdd(&dL_dview[0], dL_dnormals[idx].x * L[2].x);
		atomicAdd(&dL_dview[1], dL_dnormals[idx].y * L[2].x);
		atomicAdd(&dL_dview[2], dL_dnormals[idx].z * L[2].x);

		atomicAdd(&dL_dview[4], dL_dnormals[idx].x * L[2].y);
		atomicAdd(&dL_dview[5], dL_dnormals[idx].y * L[2].y);
		atomicAdd(&dL_dview[6], dL_dnormals[idx].z * L[2].y);

		atomicAdd(&dL_dview[8], dL_dnormals[idx].x * L[2].z);
		atomicAdd(&dL_dview[9], dL_dnormals[idx].y * L[2].z);
		atomicAdd(&dL_dview[10], dL_dnormals[idx].z * L[2].z);
		
		// Calculate dL_dprojmatrix
		glm::mat3x4 dL_dP = M * dL_dT;
		// dL_dw2n_0 = float(W) / 2.0 * dL_dP[0];
		atomicAdd(&dL_dproj[4], float(W) / 2.0 * dL_dP[0][1]);
		atomicAdd(&dL_dproj[8], float(W) / 2.0 * dL_dP[0][2]);
		atomicAdd(&dL_dproj[12], float(W) / 2.0 * dL_dP[0][3]);
		// dL_dw2n_1 = float(H) / 2.0 * dL_dP[1];
		atomicAdd(&dL_dproj[1], float(H) / 2.0 * dL_dP[1][0]);
		atomicAdd(&dL_dproj[5], float(H) / 2.0 * dL_dP[1][1]);
		atomicAdd(&dL_dproj[9], float(H) / 2.0 * dL_dP[1][2]);
		atomicAdd(&dL_dproj[13], float(H) / 2.0 * dL_dP[1][3]);
		// dL_dw2n_2 = [0,0,0,0]
		glm::vec4 dL_dw2n_3 = dL_dP * ( glm::transpose(ndc2pix)[3] );
		atomicAdd(&dL_dproj[3], dL_dw2n_3[0]);
		atomicAdd(&dL_dproj[7], dL_dw2n_3[1]);
		atomicAdd(&dL_dproj[11], dL_dw2n_3[2]);
		atomicAdd(&dL_dproj[15], dL_dw2n_3[3]);
