...
class GaussianModel:
    ...
    def reset_opacity(self):
        opacities_new = self.inverse_opacity_activation(
            torch.min(self.get_opacity, torch.ones_like(self.get_opacity) * 0.01))  # 0.01 as default
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        self.densify_and_clone(grads, max_grad, extent, frame_idx=frame_idx)
        self.densify_and_split(grads, max_grad, extent, frame_idx=frame_idx)
    ...
    def save_ply_3dgs(self, path):
        ...
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy()
        scale = torch.cat([self._scaling.detach().cpu(),
                           torch.ones((self._scaling.shape[0], 1)) * self._scaling.detach().cpu().min()],
                          dim=-1).numpy()
